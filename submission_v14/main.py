...
try:
    ...
    class CardEmbeddingExtractor(BaseFeaturesExtractor):
        def __init__(self, observation_space: gym.spaces.Dict, embedding_dim: int = 32):
            ...
            self.num_scalars = scalars_space.shape[0] # 111 or 113
...
def agent(obs_dict, config=None):
    ...
    try:
        ...
        if global_model is None and MaskablePPO is not None:
            ...
            global_model = MaskablePPO.load(model_path, custom_objects=custom_objects)
        ...
        if global_model is not None:
            ...
            for _ in range(max(1, min_count)):
                ...
                semantic_action, _ = global_model.predict(vec, action_masks=current_mask, deterministic=True)
                sem_int = int(semantic_action)
